# CAPMVM :heart:s your contributions

Thank you for taking the time to contribute to `CAPMVM`.
...
<!-- toc -->
- [CAPMVM :heart:s your contributions](#capmvm-hearts-your-contributions)
...
- [Submitting PRs](#submitting-prs)
  ...
  - [Developing CAPMVM](#developing-capmvm)
...
We are happy to see people in pretty much all areas of CAPMVM's development.
...
1. **Bump to the latest version of CAPMVM** to see whether your issue has already been fixed.
...
1. **Provide a reproduction**, for example the cluster definition yaml you are using,
  or a config file, and anything important in the environment where `CAPMVM` is running.
...
If you are having trouble using CAPMVM, but you don't think it is a bug,
please feel free to ask for help with your setup.

While you can ask for general help with `CAPMVM` usage in the [slack channel][slack-channel],
...
```bash
git clone git@github.com:<yourusername>/cluster-api-provider-microvm.git
```
...
Follow the [Developing with Tilt guide][developing] to set up your local environment.

Before you begin writing code, you may want to have a play with `CAPMVM` to get familiar
...
Run `make help` to see all development commands. Generally you will use `make build`.
...
2. **All code contributions should be submitted with unit tests**.

3. For extra special bonus points, if you see any tests missing from the area you are
...
1. **The PR has a snappy title**. Your PR title will end up in the release notes,
  ...
  or the user value it delivers, rather than any implementation detail.
    ...
    `CAPMVM`, save that for commit messages or the PR description.
...
of CAPMVM must be recorded in the form of an [ADR](https://engineering.atspotify.com/2020/04/14/when-should-i-write-an-architecture-decision-record/).
...
1. Choose an appropriate clear and concise title (eg `Implement metadata in CAPMVM`).
